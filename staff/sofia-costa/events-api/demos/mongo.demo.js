...
client.connect()
    .then(() => {
        ...
        // users.updateOne({"_id" : ObjectId("5e57eabede2a7d8150b13bb6")}, {$set: {name: 'fula'}})

        users.updateOne(
            { _id: ObjectId('5e57eabede2a7d8150b13bb6') },
            { $set: { name: "fulaaa" } })
            .then(result => {
                const { _id: id } = result
                return users.findOne({ _id: id })
        ...
        users.deleteOne({ name: 'Menga', surname: 'Nota', email: 'menganita@gmail.com', password: '123' })
    })