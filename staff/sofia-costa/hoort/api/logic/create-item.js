...
module.exports = (colorId, name, type, subtype, growth, growthDuration, soil, temperature, bestPeriod, lightPreference) => {
    ...
    return Item.findOne({ name })
        .then(item => {
            if (item) throw new NotAllowedError(`item with name ${name} already exists`)
                ...
                item = new Item({ colorId, name, type, subtype, growth, growthDuration, soil, temperature, bestPeriod, lightPreference })
